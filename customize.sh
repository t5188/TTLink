...
set_perm_recursive /data/adb/TTLink/ 0 3005 0755 0644
  ...
  for folder in /data/adb/TTLink*; do
    ...
    if [ "$clean_name" = "$largest_folder" ]; then
      ...
      if [ -d "$folder/confs" ]; then
        ...
        cp -rf "$folder/xray/"* /data/adb/TTLink/xray/
        ui_print "- Copied contents of $folder/xray to /data/adb/TTLink/xray/"
        ui_print "- 成功还原配置文件"
      fi
      break
    fi
